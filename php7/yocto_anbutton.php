...
/*********************************************************************
 *
 *  $Id: yocto_anbutton.php 59977 2024-03-18 15:02:32Z mvuilleu $
 *
...
if (!defined('Y_ANALOGCALIBRATION_OFF')) {
    define('Y_ANALOGCALIBRATION_OFF', 0);
}
if (!defined('Y_ANALOGCALIBRATION_ON')) {
    define('Y_ANALOGCALIBRATION_ON', 1);
}
if (!defined('Y_ANALOGCALIBRATION_INVALID')) {
    define('Y_ANALOGCALIBRATION_INVALID', -1);
}
if (!defined('Y_ISPRESSED_FALSE')) {
    define('Y_ISPRESSED_FALSE', 0);
}
if (!defined('Y_ISPRESSED_TRUE')) {
    define('Y_ISPRESSED_TRUE', 1);
}
if (!defined('Y_ISPRESSED_INVALID')) {
    define('Y_ISPRESSED_INVALID', -1);
}
if (!defined('Y_INPUTTYPE_ANALOG_FAST')) {
    define('Y_INPUTTYPE_ANALOG_FAST', 0);
}
if (!defined('Y_INPUTTYPE_DIGITAL4')) {
    define('Y_INPUTTYPE_DIGITAL4', 1);
}
...
class YAnButton extends YFunction
{
    ...
    protected $_calibratedValue = self::CALIBRATEDVALUE_INVALID; // UInt31
    protected $_rawValue = self::RAWVALUE_INVALID;       // UInt31
    protected $_analogCalibration = self::ANALOGCALIBRATION_INVALID; // OnOff
    protected $_calibrationMax = self::CALIBRATIONMAX_INVALID; // UInt31
    protected $_calibrationMin = self::CALIBRATIONMIN_INVALID; // UInt31
    protected $_sensitivity = self::SENSITIVITY_INVALID;    // UInt31
    protected $_isPressed = self::ISPRESSED_INVALID;      // Bool
    protected $_lastTimePressed = self::LASTTIMEPRESSED_INVALID; // Time
    protected $_lastTimeReleased = self::LASTTIMERELEASED_INVALID; // Time
    protected $_pulseCounter = self::PULSECOUNTER_INVALID;   // UInt
    protected $_pulseTimer = self::PULSETIMER_INVALID;     // Time
    protected $_inputType = self::INPUTTYPE_INVALID;      // InputType
 ...
 * @return YAnButton  a YAnButton object allowing you to drive the analog input.
 */
function yFindAnButton(string $func): YAnButton
{
 ...
 *         if there are none.
 */
function yFirstAnButton(): ?YAnButton
{
